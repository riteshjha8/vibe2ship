  ...
  return (
    <div className="glass-card min-w-0 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/85 p-5 shadow-[0_16px_45px_-24px_rgba(15,23,42,0.75)] transition hover:-translate-y-0.5 hover:border-gold-500/40">
      ...
      <div className="mt-5 rounded-3xl bg-slate-900/80 p-4 border border-slate-800">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            className="rounded-2xl bg-gold-500 px-4 py-2 text-xs font-semibold text-slate-950 transition hover:bg-gold-400"
          >
            +10%
          </button>
        </div>
        <div className="mt-4 h-3 overflow-hidden rounded-full bg-slate-800">
          <div className="h-full rounded-full bg-gradient-to-r from-gold-300 via-gold-500 to-amber-400" style={{ width: `${progress}%` }} />
        </div>
      </div>
      ...
      {goal.status === "completed" && (
        <p className="text-xs text-gold-300 mt-5">Completed 🎉 Great work staying on track.</p>
      )}
    </div>
  );
}
