    ...
    <Shell>
      <div className="relative flex min-h-[calc(100vh-4rem)] flex-col overflow-hidden">
        <div className="pointer-events-none absolute inset-0 -z-10">
          <div className="absolute right-[-6rem] top-[-4rem] h-64 w-64 rounded-full bg-teal-400/10 blur-3xl animate-float" />
          <div className="absolute left-[-7rem] top-28 h-72 w-72 rounded-full bg-sky-400/10 blur-3xl animate-float" style={{ animationDelay: "1.6s" }} />
          <div className="absolute bottom-0 right-0 h-80 w-80 rounded-full bg-cyan-400/5 blur-3xl" />
        </div>

        <div className="flex-1 min-h-0 overflow-y-auto pr-0 pb-8">
          <div className="mb-8 rounded-[2rem] border border-white/10 bg-slate-950/80 p-8 shadow-[0_30px_80px_rgba(15,23,42,0.35)] animate-slide-in-right">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h1 className="font-display text-4xl font-semibold tracking-[-0.03em] text-white">Hey {user?.name?.split(" ")[0]}</h1>
                <p className="mt-2 max-w-2xl text-sm text-slate-400">You’re in the right place to keep focus, habits, and AI-powered planning flowing smoothly.</p>
              </div>
              <button
                onClick={() => setShowForm(true)}
                className="inline-flex items-center justify-center rounded-3xl bg-teal-500 px-5 py-3 text-sm font-semibold text-ink transition hover:bg-teal-400"
              >
                + New task
              </button>
            </div>
            <div className="mt-6 flex flex-wrap gap-3">
              <span className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs uppercase tracking-[0.32em] text-teal-300">Faster daily flow</span>
              <span className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs uppercase tracking-[0.32em] text-slate-300">Smart priority insights</span>
              <span className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs uppercase tracking-[0.32em] text-slate-300">Modern dashboard</span>
            </div>
          </div>

          <div className="flex items-center justify-between mb-6 gap-4">
            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.32em] text-teal-300">Your workspace</p>
              <h2 className="font-display text-2xl font-semibold text-white">Designed for fast action and AI-guided clarity.</h2>
            </div>
            <div className="hidden sm:flex gap-2">
              <div className="h-2 w-14 rounded-full bg-teal-400/30" />
              <div className="h-2 w-10 rounded-full bg-slate-600/60" />
              <div className="h-2 w-10 rounded-full bg-slate-600/40" />
            </div>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6 reveal-on-scroll">
          ...
          <div className="grid lg:grid-cols-[2fr_1fr] gap-5 mb-8 reveal-on-scroll">
          ...
          <div className="grid xl:grid-cols-2 gap-5 mb-8 reveal-on-scroll">
            <AIStatusPanel />
            <TaskSearchPanel />
          </div>
          <div className="grid xl:grid-cols-2 gap-5 mb-8 reveal-on-scroll">
